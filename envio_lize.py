...
def processar_alunos():
    ...
    alunos_api = obter_dados_api("https://app.lizeedu.com.br/api/v2/students/", "alunos")
    turmas_api = obter_dados_api(f"https://app.lizeedu.com.br/api/v2/classes/?school_year={datetime.now().year}", "turmas")
    # Persistir alunos
    persistir_dados_em_lote("alunos_lize", alunos_api, ["id", "nome", "matricula", "email", "classes", "ativo"], "matricula")
    # Persistir turmas
    persistir_dados_em_lote("turmas_lize", turmas_api, ["id", "nome", "coordination", "school_year"], "id")
    ...
    for unidade_cod, sit, matricula, nome, turma in alunos_banco:
        ...
        try:
            with psycopg2.connect(**DB_CONFIG) as conexao:
                with conexao.cursor() as cursor:
                    cursor.execute("SELECT id, nome, email, classes, ativo FROM alunos_lize WHERE matricula = %s;", (matricula,))
        ...
        if not aluno_api:
            ...
            if inserir_aluno(nome, matricula, email_gerado):
                ...
                try:
                    with psycopg2.connect(**DB_CONFIG) as conexao:
                        with conexao.cursor() as cursor:
                            cursor.execute("SELECT id, classes FROM alunos_lize WHERE matricula = %s;", (matricula,))
                            aluno_api = cursor.fetchone()
                except Exception as e:
                    print(f"❌ Erro ao buscar aluno no banco de dados local: {e}")
                    continue
                ...
                with psycopg2.connect(**DB_CONFIG) as conexao:
                    with conexao.cursor() as cursor:
                        cursor.execute("""
                            ...
                            FROM turmas_lize t
                            LEFT JOIN alunos_lize a ON a.id = %s
...
def associar_aluno_turma(student_id, school_class_id):
    url = f"https://app.lizeedu.com.br/api/v2/students/{student_id}/set_classes/"
